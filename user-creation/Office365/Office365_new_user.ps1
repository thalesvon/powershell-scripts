...
$INPUT_FILE=Get-ChildItem 'new_365.csv'
Write-Host Criação de usuários -ForegroundColor Yellow
$body_success = @("<p>OFFICE 365 New User Account Details</p><br><br><table>
        <tr>
            <th>DisplayName    </th>
...
$ou = $_.ou
    ...
    Else{
        ...
        try{
            Enable-RemoteMailbox $sam -RemoteRoutingAddress "$sam@$remote_routing_address"
        }
        catch{
...
Invoke-Command -ComputerName $($env:aad_connect_server) -ScriptBlock {Start-ADSyncSyncCycle -PolicyType Delta}
...
$emailMessage.Subject = "OFFICE 365 - New User Created"
...
if($failed_user -eq $true){
    ...
    $emailMessage.From = " $($env:mail_user_name) <$($env:mail_user_email)>"
    $emailMessage.To.Add( "$($env:mail_user_email)")
    $emailMessage.Subject = "FAILED OFFICE 365 - Users not created"
    $emailMessage.IsBodyHtml = $true
